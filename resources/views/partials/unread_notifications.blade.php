...
<li>
    ...
    @else
    <a class="dropdown-item update-notification-status" data-id="{{ $notification->id }}" href="{{ $notification->type == 'project' ? url('/projects/information/' . $notification->type_id) : ($notification->type == 'task' ? url('/tasks/information/' . $notification->type_id) : ($notification->type == 'workspace' ? url('/workspaces') : url('/meetings'))) }}">
        <div class="d-flex align-items-center">
            <div class="me-auto fw-semibold">{{$notification->title}} <small class="text-muted mx-2">{{ $notification->created_at->diffForHumans() }}</small></div>
            <i class="bx bx-bell me-2"></i>
        </div>
