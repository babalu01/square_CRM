...
<div class="<?= $flag == 1 ? 'card ' : '' ?>mt-2">
    ...
    <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                <select class="form-control projects_select" id="task_project_filter" multiple="multiple" data-placeholder="<?= get_label('select_projects', 'Select Projects') ?>">
                </select>
            </div>
            @endif
            ...
            <div class="col-md-4 mb-3">
                <select class="form-control users_select" id="task_user_filter" name="user_ids[]" multiple="multiple" data-placeholder="<?= get_label('select_users', 'Select Users') ?>">
                </select>
            </div>
            <div class="col-md-4 mb-3">
                <select class="form-control clients_select" id="task_client_filter" name="client_ids[]" multiple="multiple" data-placeholder="<?= get_label('select_clients', 'Select Clients') ?>">>
                </select>
            </div>
            @endif
            @endif
            <div class="col-md-4 mb-3">
                <select class="form-control statuses_filter" id="task_status_filter" name="status_ids[]" multiple="multiple" data-placeholder="<?= get_label('select_statuses', 'Select Statuses') ?>">
                </select>
            </div>
            <div class="col-md-4 mb-3">
                <select class="form-control priorities_filter" id="task_priority_filter" name="priority_ids[]" multiple="multiple" data-placeholder="<?= get_label('select_priorities', 'Select Priorities') ?>">
                </select>
            </div>
        </div>
        ...
        <div class="table-responsive text-nowrap">
            ...
            <input type="hidden" id="data_reload" value="{{ request()->is('home') || request()->is('projects/information/*') ? '1' : '0' }}">
            <input type="hidden" id="save_column_visibility">
            <input type="hidden" id="multi_select">
            <table id="task_table" data-toggle="table" data-loading-template="loadingTemplate" data-url="{{ isset($viewAssigned) && $viewAssigned == 1 ? '' : (!empty($id) ? url('/tasks/list/' . $id . '?from_home=' . (request()->is('home') || request()->is('projects/information/*') ? '1' : '0')) : url('/tasks/list?from_home=' . (request()->is('home') || request()->is('projects/information/*') ? '1' : '0'))) }}" data-icons-prefix="bx" data-icons="icons" data-show-refresh="true" data-total-field="total" data-trim-on-search="false" data-data-field="rows" data-page-list="[5, 10, 20, 50, 100, 200]" data-search="true" data-side-pagination="server" data-show-columns="true" data-pagination="true" data-sort-name="id" data-sort-order="desc" data-mobile-responsive="true" data-query-params="queryParamsTasks">
                <thead>
                    <tr>
                        ...
                        <th data-field="due_date" data-visible="{{ (in_array('end_date', $visibleColumns) || empty($visibleColumns)) ? 'true' : 'false' }}" data-sortable="true">{{ get_label('ends_at', 'Ends at') }}</th>
