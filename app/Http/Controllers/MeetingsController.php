...
class MeetingsController extends Controller
{
    ...
    public function list()
    {
        ...
        $meetings = $meetings->orderBy($sort, $order)
            ...
            ->through(function ($meeting) use ($canEdit, $canDelete, $canCreate, $currentDateTime) {
                ...
                if ($status == 'Ongoing') {
                    $actions .= '<a href="' . url("/meetings/join/{$meeting->id}") . '" target="_blank" title="Join">' .
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            // Set creator as a participant automatically if not already included
    ...
    {
        return response()->json(['error' => false, 'message' => 'Join Meeting API Test.']);
        $meeting = Meeting::findOrFail($id);
        $currentDateTime = Carbon::now(config('app.timezone'));
