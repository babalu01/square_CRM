    ...
    public function list()
    {
        ...
        $leave_requests = $leave_requests->orderBy($sort, $order)
            ...
            ->through(function ($leave_request) use ($isAdmin, $isAdminOrLeaveEditor) {
                ...
                return [
                    'id' => $leave_request->id,
                    'user_name' => $leave_request->user_name . "<ul class='list-unstyled users-list m-0 avatar-group d-flex align-items-center'><a href='" . url("/users/profile/{$leave_request->user_id}") . "' target='_blank'><li class='avatar avatar-sm pull-up' title='{$leave_request->user_name}'>
