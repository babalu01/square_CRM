    ...
    "
        ...
        $activity_log = $activity_log_query->get()->map(function ($activity_log) use ($canDelete, $isApi) {
            ...
            $activity_log_data = [
                ...
                'actor_id' => $activity_log->actor_id,
                'actor_name' => $isApi
                    ? $activity_log->actor_name : '<div class="d-flex align-items-center">
                    <div class="avatar avatar-sm pull-up" title="' . $activity_log->actor_name . '">
                        <a href="' . ($activity_log->actor_type == 'user'
                            ? route('users.profile', ['id' => $user->id])
                            : route('clients.profile', ['id' => $user->id])) . '">
        ...
        if ($isApi) {
            return response()->json([
                "error" => $activity_log->isEmpty() ? true : false,
