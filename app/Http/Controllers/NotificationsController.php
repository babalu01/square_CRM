...
class NotificationsController extends Controller
{
    ...
    public function list()
    {
        ...
        $notifications->through(function ($notification) use ($canDelete) {
            ...
            if ($notification->action != 'team_member_on_leave_alert') {
        ...
        foreach ($notifications->items() as $notification => $collection) {
            foreach ($collection['clients'] as $i => $client) {
                $collection['clients'][$i] = "<a href='" . url("/clients/profile/{$client->id}") . "' target='_blank'><li class='avatar avatar-sm pull-up' title='{$client['first_name']} {$client['last_name']}'>
        ...
        foreach ($notifications->items() as $notification => $collection) {
            foreach ($collection['users'] as $i => $user) {
                $collection['users'][$i] = "<a href='" . url("/users/profile/{$user->id}") . "' target='_blank'><li class='avatar avatar-sm pull-up' title='{$user['first_name']} {$user['last_name']}'>
